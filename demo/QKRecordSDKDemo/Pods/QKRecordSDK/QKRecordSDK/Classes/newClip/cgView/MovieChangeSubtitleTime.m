...
@property (nonatomic, strong) SubTitleSelectTimeView *recordView;
@end

@implementation MovieChangeSubtitleTime

-(instancetype)initWithFrame:(CGRect)frame{
    self = [super initWithFrame:frame];
    if(self){
        self.nowArrayType = 0;
        self.v_imgs = [[UIView alloc] initWithFrame:CGRectMake(vMarginLeft, (frame.size.height - imgHeight)/2, vAllWidth, imgHeight)];
        self.v_imgs.clipsToBounds = YES;
        [self addSubview:self.v_imgs];
        self.v_imgs.backgroundColor = [UIColor colorWithRed:39.0/255.0 green:39.0/255.0 blue:39.0/255.0 alpha:1.0];
        
        
        self.v_audioRecord= [[UIView alloc] initWithFrame:CGRectMake(vMarginLeft, (frame.size.height - imgHeight)/2, vAllWidth, imgHeight)];
        ...
        float x_true =  vMarginLeft - 30/2;
        
        
        self.img_line = [[UIImageView alloc] initWithFrame:CGRectMake(x_true, 0, 30, imgHeight)] ;
        self.img_line.image = [clipPubthings imageNamed:@"qk_moviepartimgs_midsil"];
...
-(void)setStartTime:(double )startTime during:(double)during{
    int orgX = startTime*vAllWidth/self.allTime;
    int width = during*vAllWidth/self.allTime;
    if(width < 2){
        width = 2;
    }
    SubTitleSelectTimeView *audioview = [[SubTitleSelectTimeView alloc] initWithFrame:CGRectMake(orgX, 0, width, self.frame.size.height)];
    
    audioview.startTime = startTime;
    audioview.endTime = startTime + during;
    [self.v_audioRecord addSubview:audioview];
    self.recordView = audioview;
    
    
    float x_true =  orgX + width- self.img_line.frame.size.width/2;
    
    
    self.img_line.frame =CGRectMake(x_true + vMarginLeft, 0, 30, imgHeight);
}


-(double)getEndTime{
    return self.recordView.endTime;
}
-(double)getStartTime{
    return self.recordView.startTime;
}
-(double)getDuringTime{
    return self.recordView.endTime - self.recordView.startTime;
}
-(double)getAllTime{
    return _allTime;
}
...
-(NSArray*)getMoviesArray:(NSArray*)arrayMovies{
    ...
    for(int i = 0;i < arrayMovies.count ;i++){
         ...
         */
        int nowAll = (during + part.beginTime)*allImgsCount/ self.allTime;
        //最后一个片段要把所有的图片分配完
...
- (void)move:(CGPoint)point {
    ...
    if(time - self.recordView.startTime <=3){
        time = self.recordView.startTime + 3;
        int orgX = time*vAllWidth/self.allTime;
        x =  orgX - self.img_line.frame.size.width/2 + vMarginLeft;
    ...
    if(changeTime){
        changeTime(time - self.recordView.startTime);
    }
    
    self.recordView.endTime = time;
    int width = (time - self.recordView.startTime)*vAllWidth/self.allTime;
    self.recordView.frame = CGRectMake(self.recordView.frame.origin.x, 0, width, self.recordView.frame.size.height);
}


-(void)setEndTime:(double)playtime{
    if(playtime > self.allTime){
        playtime = self.allTime;
    }else if(playtime < self.recordView.startTime + 3){
        playtime = self.recordView.startTime + 3;
    }
    float x_true =  vMarginLeft - 30/2 + playtime/self.allTime * vAllWidth;;
    self.img_line.frame = CGRectMake(x_true, 0, 30, imgHeight);
...
//    int counts = 0;
//    if(counts == 0){
//        counts = (self.frame.size.width - 16)/imgWidth;
//    }
    return 8;
}
